...
" commands
syntax keyword ksCommands contained auth authconfig autopart autostep bootloader btrfs cdrom clearpart cmdline container_boot_image container_boot_options container_registries container_storage device deviceprobe displaymode dmraid driverdisk eula fcoe firewall firstboot graphical group halt harddrive ignoredisk install interactive iscsiname iscsi keyboard key lang langsupport lilocheck liveimg logging logvol mediacheck monitor mouse multipath network nfs ostreesetup part partition poweroff raid realm reboot repo reqpart rescue rootpw selinux services shutdown skipx sshkey sshpw text timezone unsupported_hardware updates upgrade url user vnc volgroup xconfig zerombr zfcp
